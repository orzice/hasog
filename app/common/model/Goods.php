...
//                            }
    ...
    public function calculate_credit(Member $user, array $credit_type_array, $goods_num=1){
        $msg = -1;
...
//            $need_credit =  $this->deduction_rate/100 ;
            ...
            if ($user_credit >= $need_credit){
                ...
                $user->setAttr($user->getAttr($credit_type->value), $user->getAttr($credit_type->value) - $need_credit);
//                $credit_type_array[$credit_type->value] = isset($credit_type_array[$credit_type->value]) ? $credit_type_array[$credit_type->value] + $credit_amount : $credit_amount;
                ...
                $user->setAttr($user->getAttr($credit_type->value), 0);
                $msg = 2;
            }
...
//        $this->total_transfer_amount = $could_amount;// 总积分抵扣的金额
        $this->old_price = $this->price; // 扣除积分抵扣前商品价格
        $this->price = $this->price * $goods_num - round($could_amount,2); // 总扣除积分抵扣后商品价格
